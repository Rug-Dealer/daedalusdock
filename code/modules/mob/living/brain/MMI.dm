...
/obj/item/mmi/attackby(obj/item/O, mob/user, params)
	...
	if(istype(O, /obj/item/organ/brain)) //Time to stick a brain in it --NEO
		...
		else if(!fubar_brain && newbrain.organ_flags & ORGAN_DEAD) // the brain is damaged, but not from a suicider
...
/obj/item/mmi/examine(mob/user)
	. = ..()
	if(radio)
		. += span_info("There is a switch to toggle the radio system [radio.is_on() ? "off" : "on"].[brain ? " It is currently being covered by [brain]." : null]")
	if(brainmob)
		var/mob/living/brain/B = brainmob
		if(!B.key || !B.mind || B.stat == DEAD)
			. += span_alert("\The [src] indicates that the brain is completely unresponsive.")
		else if(!B.client)
			. += span_alert("\The [src] indicates that the brain is currently inactive; it might change.")
		else
			. += span_info("\The [src] indicates that the brain is active.")
...
/obj/item/mmi/proc/brain_check(mob/user)
	...
	if(B.stat == DEAD)
		...
		return FALSE
	if(brain?.organ_flags & ORGAN_DEAD)
		if(user)
